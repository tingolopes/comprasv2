...
def coletar():
    ...
    print(f"🚀 Iniciando coleta dinâmica de amostras (Padrão temp_)...")

    # Localiza todas as pastas que começam com 'temp_' usando Regex
    diretorio_atual = os.getcwd()
    pastas_detectadas = [d for d in os.listdir(diretorio_atual)
                         if os.path.isdir(d) and re.match(r'^temp_', d)]

    if not pastas_detectadas:
        print("⚠️ Nenhuma pasta começando com 'temp_' foi encontrada.")
        return

    for pasta_origem in pastas_detectadas:
        # O rótulo será o nome da pasta em maiúsculas sem o prefixo temp_ para ficar limpo
        rotulo = pasta_origem.replace("temp_", "").upper()
