...
def verificar_sucesso(caminho):
    """Dá skip apenas se o arquivo foi atualizado hoje e é válido."""
...
def processar_tarefa(t):
    """Thread segura: Lida com respostas nulas da API sem explodir."""
    arquivo = f"{PASTA_ATAS}/atas_{t['sigla']}_{t['ano']}_p{t['pagina']}.json"

    sucesso_cache, dados_cache = verificar_sucesso(arquivo)

    # --- AJUSTE: Acesso seguro ao cache ---
    if sucesso_cache and dados_cache:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            salvar_dados(arquivo, BASE_URL, params, dados_api, "SUCESSO")
            # --- AJUSTE: Acesso seguro à resposta da API ---
            pag_rest = dados_api.get("respostas", {}).get(
                "paginasRestantes", 0) if isinstance(dados_api, dict) else 0
            return f"✅ DONE | {t['sigla']} | {t['ano']} | Pág {t['pagina']}", pag_rest

        else:
            # Em caso de erro na API, tenta recuperar a info de páginas do cache antigo
            salvar_dados(arquivo, BASE_URL, params, None,
        ...
        while futures:
            ...
            for future in done:
                ...
                try:
                    ...
                    print(f"⚠️ Erro crítico na thread: {exc}")
                    falhas_restantes += 1
